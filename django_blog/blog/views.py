...
# Update your PostDetailView to include comments + form
class PostDetailView(DetailView):
    model = Post
    template_name = 'blog/post_detail.html'
    context_object_name = 'post'
...
class CommentCreateView(LoginRequiredMixin, CreateView):
    ...
    template_name = 'blog/comment_form.html'  # fallback if you want a standalone page
...
class CommentUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        return comment.author == self.request.user and comment.post.pk == int(self.kwargs.get('post_pk'))
...
class TagPostListView(ListView):
    model = Post
    template_name = 'blog/post_list.html'  # reuse list template for tag results
    context_object_name = 'posts'
    paginate_by = 10
    ...
    def get_queryset(self):
        ...
        # return posts related to tag, newest first
...
# Search view (simple function view)
def search_view(request):
    q = request.GET.get('q', '').strip()
    posts = Post.objects.none()
    if q:
        # search title or content (case-insensitive) OR tag name
